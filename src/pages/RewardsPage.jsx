...
const rewardFilters = ['All Categories', 'Physical Goods', 'Donations', 'Digital Cards']
...
function RewardCard({ item }) {
  return (
    <article className="reward-card">
      <div className="reward-card-media" style={{ backgroundImage: `url(${item.image})` }}>
        <div className="reward-card-chip-row">
          {item.tag ? <span className="reward-chip">{item.tag}</span> : null}
          <button type="button" className="reward-card-menu" aria-label="More options">⋮</button>
        </div>
      </div>
      ...
      <div className="reward-card-body">
        ...
        <div className="reward-card-footer">
          <button type="button" className={`status-toggle${item.status === 'Active' ? ' active' : ''}`}>
            {item.status}
          </button>
          <button type="button" className="reward-edit">Edit</button>
        </div>
      </div>
    </article>
  )
...
function RewardsPage() {
  return (
    <section className="rewards-page">
      ...
      <section className="rewards-toolbar" aria-label="Rewards filters and sort">
